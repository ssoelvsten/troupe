...
function constructCurrent(compilerOutput: string) {
    ...
    const snippets = compilerOutput.split("\n\n");
    let k = 0;


    const ctxt = { // deserialization context
    ...
    for (let i = 0; i < serobj.namespaces.length; i++) {
        ...
        for (let j = 0; j < ns.length; j++) {
            ...
            for (const atom of snippetJson.atoms) {
                atomSet.add(atom);
            }
            // console.log (snippetJson.atoms)
        }
        const argNames = Array.from(atomSet);
        const argValues = argNames.map( argName => {return new Atom(argName);});
        argNames.unshift('rt');
        argNames.push(nsFun);
    ...
    function mkEnv(i: number, post_init?: (any)=>void ) {
        if (!ctxt.envs[i]) {
            const env = {__dataLevel : levels.BOT};
            if (post_init) {
                post_init (env);
            }
            ctxt.envs[i] = env;
            for (const field in serenvs[i]) {
