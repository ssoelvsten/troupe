...
const isPiniMode = argv[TroupeCliArg.Pini]?true:false;
...
export class SleepTimeout {
    ...
    afterTimeout () {
        const theThread = this.resumeThread;
        theThread.sleepTimeout = null;
        theThread.returnSuspended (__unit);
        const sched = getRuntimeObject().__sched;
    ...
    private _validateDowngradeOrThrow(
        ...
        const downgradeCheckResult: DowngradeResult =
            levels.okToDowngrade(params.downgradeKind, params.downgradeDimension)
                 (params.levFrom, params.levTo, params.authorityLevel, params.blockLevel);
...
//        this._sp += FRAMESIZE;
    ...
    returnSuspended (arg) {
        ...
        const rv = new LValCopyAt (arg, this.pc);
        this.next = () => {
    ...
    returnImmediate () {
        const _STACK = this.callStack;
        const _SP = this._sp;

        const branchFlag = _STACK[_SP  - BRANCHFLAGOFFSET];
        const lclear = _STACK[_SP - MCLEAROFFSET];
        const ret = _STACK[_SP - RETOFFSET];
    ...
    pcpinipush ( auth: any, purpose: PCDeclassificationPurpose | string, bl = this.bl )  {
        const uid = uuidv4();
        const cap = this.mkVal (new Capability(uid,
                    { bl
                    , pc: this.pc
                    , auth : auth
                    , purpose: purpose
    ...
    pcpop (cap_lval) {
        ...
        const cap: Capability<any> = cap_lval.val;
        const {bl, pc, auth, purpose} = cap.data;
    ...
    blockEndorseTo (auth, bl_to = this.pc) {
        ...
        const ok_to_use = levels.flowsTo (auth.lev, bl_to);
        if (!ok_to_use) {
    ...
    */
    ...
    raiseMboxClearance (new_lclear: any) {
        ...
        const uid = uuidv4() ;
        const cap = this.mkVal (new Capability(uid, this.mailbox.mclear, this.mailbox.caps, this.pc));
    ...
    lowerMboxClearance (cap_lval:any, auth:any) {
        ...
        const cap:Capability<MboxClearance> = cap_lval.val;
