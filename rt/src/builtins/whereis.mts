...
export function BuiltinRegistry<TBase extends Constructor<UserRuntimeZero>>(Base: TBase) {
    return class extends Base {
        ...
        whereis = mkBase((arg) => {
            ...
            const __sched = $r.__sched;

            // let n = dealias(arg.val[0].val);
            const n = __nodeManager.getNode(arg.val[0].val).nodeId;

            const k = arg.val[1].val;
            const nodeLev = nodeTrustLevel(n);
            const theThread = $r.$t;


            const okToLookup = flowsTo(lub($r.$t.pc, arg.val[0].lev, arg.val[1].lev), nodeLev);
            if (!okToLookup) {
            ...
            if (__nodeManager.isLocalNode(n)) {
                ...
                (async () => {
                    try {
                        const body1 = await p2p.whereisp2p(n, k);
                        const body = await deserialize(nodeTrustLevel(n), body1);
