...
export function BuiltinStdIo<TBase extends Constructor<UserRuntimeZero>>(Base: TBase) {
    return class extends Base {
        getStdout = mkBase((arg) => {
            ...
            const sufficentAuthority = flowsTo(__stdio_lev, arg.val.authorityLevel);
            if (sufficentAuthority) {
                const obj = new LocalObject(process.stdout);
        ...
        fprintln = mkBase((arg) => {
            ...
            const out = arg.val[0].val._value;
        ...
        inputLine = mkBase((arg) => {
            ...
            if (lineBuffer.length > 0) {
                const s = lineBuffer.shift();
                const r = theThread.mkValWithLev(s, __stdio_lev);
