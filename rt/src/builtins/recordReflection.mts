...
export function BuiltinRecordReflection<TBase extends Constructor<UserRuntimeZero>>(Base: TBase) {
    return class extends Base {
        recordExtend = mkBase((larg) => {
            ...
            const raw_rec = arg[0].val;
            assertIsString(arg[1]);
            const new_field_str = arg[1];
            const raw_newfield = new_field_str.val;
            const v = new LVal (arg[2].val, lub (arg[2].lev , new_field_str.lev));
            const new_raw_rec = Record.mkWithRecord (raw_rec, [[raw_newfield, v ]]);
            const ret_val = new LVal (new_raw_rec, lub (this.runtime.$t.pc, larg.lev, new_field_str.lev));
