...
export async function loadLibsAsync(obj, rtObj) {
    ...
    for (let n = 0; n < libs.length; n++) {
        const lib = libs[n].lib;
        const decl = libs[n].decl;
        ...
        const filename = process.env.TROUPE + "/lib/out/" + lib + ".js";
        ...
        const Lib: any = new Function('rt', input);

        // 4. We create a "new" instance of the resulting class

        const libinstance = new Lib(rtObj);
        ...
        const table = libinstance.export({__dataLevel:levels.BOT}).val.toArray();
