...
import Test.Tasty.Golden (findByExtension, goldenVsString, goldenVsStringDiff)
...
runTimeout n testname tc = do
    ...
    readProcessWithExitCode
        timeout
        ([show n, "./local.sh"] ++ mkRunArgs tc ++ [testname])
        ""
...
goldenTests tc = do
    ...
    divergingTestsForRuntime <-
        findByExtension extensions "tests/rt/timeout/diverging"
    testsForLib <- findByExtension extensions "tests/lib"

    return $
        ( testGroup ("Troupe golden tests (" ++ ppTestConfig tc ++ ")") $
            map
                ($ tc)
                [ compilerTests negativeTestsForCompiler
                , runtimeTests $
                    concat
                        [positiveTestsForRuntime, negativeTestsForRuntime, warningTestsForRuntime]
