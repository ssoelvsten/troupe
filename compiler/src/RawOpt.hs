...
data PState
    = PState
    { stateMon :: Map MonComponent RawVar -- monitor state
...
pevalInst i = do
    ...
    case i' of
        ...
        AssignRaw r (Bin Basics.LatticeJoin (UseNativeBinop False) x y) -> do
            if x == y
                then _omit (addSubst r x) -- trivial join
                else do
...
filterInstBwd ls =
    let f (pc, bl) (i : is) acc =
        ...
        f (pc, bl) [] acc =
            let fromJ (Just x) = [x]
                fromJ Nothing = []
             in (acc, concat $ map fromJ [pc, bl])
     in f (Nothing, Nothing) (reverse ls) []
