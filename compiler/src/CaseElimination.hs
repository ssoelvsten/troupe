...
trans mode (S.Prog imports atms tm) = do
    let tm' = case mode of
            Normal ->
                S.Let
                    [S.ValDecl (S.VarPattern "authority") (S.Var "$$authorityarg") _srcRT]
                    tm
            Export -> tm
...
transHandler (S.Handler pat1 mbpat2 guard body) = do
    let argInput = "$input"
        ...
        lamBody =
            S.Case
                (S.Var argInput)
                [(S.TuplePattern [pat1, pat2], guardCheck), (S.Wildcard, callFailure)]
                _srcRT
...
-}
compilePattern ::
    T.Term -> (T.Term, S.DeclPattern) -> ReaderT T.Term Trans T.Term
    ...
    return $
        ifpat
            ( Bin
                And
                (Un IsTuple v)
                (Bin Eq (Un TupleLength v) (Lit (LInt (toInteger (length pats)) _srcRT)))
            )
            succ'
            fail
    ...
    return $
        ifpat
            ( Bin
                And
                (Un IsList v)
                (Bin Eq (Un ListLength v) (Lit (LInt (toInteger (length pats)) _srcRT)))
            )
            succ'
            fail
compilePattern succ (v, S.ConsPattern p1 p2) = do
    ...
    return $
        ifpat
            (Bin And (Un IsList v) (Bin Gt (Un ListLength v) (Lit (LInt 0 _srcRT))))
            succ''
            fail
compilePattern succ (v, S.RecordPattern fieldPatterns mode) = do
    ...
    if not (null duplicates)
        then
            lift $
                throwError $
                    "Duplicate field names in record pattern: " ++ show duplicates
        else do
...
transDecl (S.ValDecl pat t pos) succ = do
    ...
    result <-
        runReaderT
            (compilePattern succ ((Var temp), pat))
...
transDecl (S.FunDecs fundecs) succ = do
  ...
  where
    ...
    transFunDecl (S.FunDecl f lams pos) = do
        let lams' =
                map
                    (transLambda_aux . (\(S.Lambda args e) -> S.Lambda [S.TuplePattern args] e))
                    lams
...
transTerm (S.Case t cases pos) = do
    ...
    let e =
            foldr
                ...
                (Error (Lit (LString "pattern match failure in case expression")) pos)
                cases'
