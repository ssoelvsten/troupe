...
offsetMap consts (DefUse{defs, uses, escapingUses}) =
    let isEscaping x (c_def, _) =
            let uses_set = Map.findWithDefault (Set.empty) x uses
             ...
             in (0 < Set.size uses')
                    && ( case x of
                            AssignableRaw r -> Map.notMember r consts
                            _ -> True
                       )
...
instance (Usable a b) => Usable [a] b where
    use x = mapM_ use x
...
updateZone i = do
    ...
    let zoneType = zoneCounter `mod` 2 == 0
        ...
        typeAsBool _ = True
    if (typeAsBool . instructionType) i /= zoneType
        then
