  ...
  where
    ...
    translateGroup insts = do
        ...
        let __uses = (uses . defsUses) rr
            ...
            prologue =
                [ Stack.AssignRaw Stack.AssignLet x (Raw.ProjectState Raw.MonPC)
                | x <- assignVars
                , isGroupEscaping x
                ]

            epilogue =
                [ Stack.StoreStack x' (rel j)
                | x <- assignVars
                , isBlockEscaping x
                , let x' = Raw.AssignableRaw x
            ...
            tri i = case i of
                Raw.AssignRaw x y ->
                    let t =
                            if isGroupEscaping x
                                then Stack.AssignMut
                                else Stack.AssignConst
                     in Stack.AssignRaw t x y
                Raw.SetState cmp x -> Stack.SetState cmp x
                _ ->
                    error
                        "impossible case/bug: only label instructions must be passed to this translation function"
...
trTr (Raw.Call bb1 bb2) = do
    ...
    let filterConsts (Raw.AssignableRaw x) = Map.notMember x consts
        filterConsts _ = True
    let loads =
            [ Stack.FetchStack x (rel (Map.findWithDefault (error (show x)) x offsets))
