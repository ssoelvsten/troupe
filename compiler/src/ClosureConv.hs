...
type CCEnv =
    (CompileMode, C.Atoms, NestingLevel, Map VarName VarLevel, Maybe VarName)
...
mkEnvBindings fv = do
    ...
    let envVars =
            (map (\(v, _) -> (v, VarLocal v)) boundVars)
...
cpsToIR (CPS.LetSimple vname@(VN ident) st kt) = do
    i <-
        let _assign arg = return $ Just $ CCIR.Assign vname arg
         in case st of
                ...
                CPS.ValSimpleTerm (CPS.Lit lit) -> do
                    lev <- askLev
                    tell ([], [], [((vname, lit), lev)])
                    return Nothing
