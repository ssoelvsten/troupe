...
ppLibAccess (LibAccess (Basics.LibName libname) varname) =
    PP.braces $
        PP.text "lib:"
            <+> (PP.doubleQuotes . PP.text) libname
            <+> PP.text ","
            <+> PP.text "decl:"
            <+> (PP.doubleQuotes . PP.text) varname
    ...
    let (inner, _, (libs, atoms, konts)) = runRWS (toJS x) False initState
     in Aeson.encode $
            JSOutput
                { libs = libs
                , fname = case x of
                    FunStackUnit (FunDef (HFN n) _ _ _ _) -> Just n
                    _ -> Nothing
                , atoms = atoms
                    ...
                    ( map
                        ( \a ->
                            hsep
                                ...
                                , text a
                                , "= new rt.Atom"
                                , (PP.parens ((PP.doubleQuotes . text) a))
                                ]
                        )
                        atoms
                    )
...
lit2JS (C.LDCLabel dc) =
    text "rt.mkDCLabel"
        <> (PP.parens . text . jsonValueToString . toJSON . dcLabelExpToDCLabel) dc
lit2JS lit = ppLit lit

constsToJS consts =
    vcat $ map toJsConst consts
  where
    toJsConst (x, lit) = hsep ["const", ppId x, text "=", lit2JS lit]
...
instance ToJS FunDef where
    toJS fdef@(FunDef hfn stacksize consts bb irfdef) = do
        ...
        modify
            ( \s ->
                s{frameSize = _frameSize, sparseSlot = stacksize, stHFN = hfn, consts = consts} -- + 1 for the sparse flag; 2021-03-17; AA
            )
        ...
        return $
            vcat
                [ text "this." PP.<> ppId hfn <+> text "=" <+> ppArgs ["$env"] <+> text "=> {"
                , if debug
                    then nest 2 $ text "rt.debug" <+> (PP.parens . PP.doubleQuotes . ppId) hfn
                    else PP.empty
                , nest 2 $
                    vcat $
                        [ "let _T = rt.runtime.$t"
                ...
                , semi $
                    text "this."
                        PP.<> ppId hfn
                        PP.<> text ".serialized ="
                        <+> (pickle . serializeFunDef) irfdef
                , semi $
                    text "this." PP.<> ppId hfn PP.<> text ".framesize =" <+> (PP.int stacksize)
                ]
...
ir2js (MkFunClosures envBindings funBindings) = do
  ...
  where
    ppEnvIds :: VarName -> [(VarName, IR.VarAccess)] -> W PP.Doc
    ppEnvIds env ls = do
        let penv = ppId env
        d1 <-
            mapM
...
ir2js (LabelGroup ii) = do
  ...
  where
    ppLevelOp (AssignRaw tt vn e) = do
        jj <- toJS e
...
instance ToJS RawExpr where
    toJS x = do
        ...
        case x of
            ...
            ProjectLVal IR.VarFunSelfRef lf ->
                return
                    ( case lf of
                        Raw.FieldValue ->
                            ppFunSelfRef
                                PP.<> text "."
                                PP.<> PP.text (show Raw.FieldValue)
                        Raw.FieldValLev -> monStateToJs MonPC
                        Raw.FieldTypLev -> monStateToJs MonPC
                    )
            e@(ProjectLVal _ _) -> return $ ppRawExpr e
            Bin binop use_native va1 va2 ->
                return $
                    let text' = text (binOpToJS binop use_native)
                     in if isInfixBinop binop use_native
            ...
            Tuple vars ->
                return $
                    text "rt.mkTuple"
                        <> PP.parens (PP.brackets $ PP.hsep $ PP.punctuate (text ",") (map ppVarName vars))
            Record fields -> do
                jsFields <- fieldsToJS fields
                return $
                    PP.parens $
                        text "rt.mkRecord" <> PP.parens (PP.brackets $ PP.hsep $ jsFields)
            ...
            Lib lib'@(Basics.LibName libname) varname -> do
                ...
                return $
                    text "rt.loadLib"
                        <> PP.parens
                            ( (PP.doubleQuotes . text) libname
                                <> text ", "
                                <> (PP.doubleQuotes . text) varname
                                <> text ", this"
                            )
...
jsClosure var env f =
    vcat
        [ ppLet var
            <+> ( (text "rt.mkVal")
                    <> ( PP.parens
                            ( (text "rt.RawClosure")
                                <> ( PP.parens
                                        (PP.hsep $ PP.punctuate "," [ppId env, text "this", text "this." PP.<> ppId f])
                                   )
                            )
                       )
                )
        ...
        , semi $
            ppId env PP.<> PP.text "." PP.<> (ppId var) PP.<> text ".selfpointer = true"
        ]
