...
visitTerm atms (Let decls term) =
  ...
  where
    ...
    visitDecl (FunDecs decs) =
        FunDecs
            ( map
                (\(FunDecl nm lams pos) -> (FunDecl nm (map (visitLambda atms) lams) pos))
                decs
            )
visitTerm atms (Case t declTermList p) =
    Case
        (visitTerm atms t)
        ( map
            (\(pat, term) -> ((visitPattern atms pat), (visitTerm atms term)))
            declTermList
        )
        p
...
visitFields atms fs = map visitField fs
  where
    visitField (f, Nothing) = (f, Nothing)
