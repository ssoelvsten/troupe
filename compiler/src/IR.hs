...
newtype HFN = HFN Ident deriving (Eq, Generic, Ord, Show)
...
data IRBBTree = BB [IRInst] IRTerminator deriving (Eq, Generic, Show)
...
ppDeps a =
    let (ffs_0, lls_0, atoms_0) = execWriter (dependencies a)
...
instance WellFormedIRCheck IRExpr where
    ...
    wfir (ProjIdx _ idx) =
        when (idx > (fromIntegral Consts.llvm_maxIndex :: Word)) $
            throwError $
                "ProjIdx: illegal index: "
                    ++ show idx
                    ++ " (max index: "
                    ++ show Consts.llvm_maxIndex
                    ++ ")"
    wfir _ = return ()
...
wfFun (FunDef (HFN fn) (VN arg) consts bb) =
    let initVars = [fn, arg] ++ [i | VN i <- fst (unzip consts)]
        act = do
            ...
            wfir bb
     in case evalState (runExceptT act) [] of
            Right _ -> return ()
            Left s -> throwError s
...
ppConsts consts =
  ...
  where
    ppConst (x, lit) = hsep [ppId x, text "=", ppLit lit]

ppFunDef (FunDef hfn arg consts insts) =
    vcat
        [ text "func" <+> ppFunCall (ppId hfn) [ppId arg] <+> text "{"
...
ppIRExpr (Base v) =
    if v == "$$authorityarg" -- special casing; hack; 2018-10-18: AA
        then text v
