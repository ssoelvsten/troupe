...
getR0 = do
    ...
    tell
        [ AssignRaw rVal (ProjectState R0_Val)
...
_raisePC raiseBy = do
    ...
    pc' <- freshRawVarWith "_pc_"
    tell
        [ AssignRaw pc' (_default_bin Basics.LatticeJoin pc raiseBy)
        , SetState MonPC pc'
        ]
...
_raiseBlock raiseBy = do
    ...
    bl' <- freshRawVarWith "_bl_"
    tell
        [ AssignRaw bl' (_default_bin Basics.LatticeJoin bl raiseBy)
...
expr2rawComp = \case
    ...
    IR.Base "$$authorityarg" ->
        return $
            let v = IR.VarLocal (VN "$$authorityarg")
             in SimpleRawComp
                    { cVal = RVar v
                    , cValLbl = ValLbl v
                    , cTyLbl = TyLbl v
...
fun2raw irfdef@(IR.FunDef hfn vname consts (IR.BB irInsts irTr)) =
  ...
  where
    ((tr, rawConsts), insts) = evalRWS comp () 0
    comp = do
