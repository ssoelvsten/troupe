    ...
    if eof
        then exitSuccess
            ...
            if BS.isPrefixOf "!ECHO " input
                then
                    let response = BS.drop 6 input
                     in do BSChar8.putStrLn response
                --                  debugOut "echo"
                else case decode input of
                    Right bs ->
                        case CCIR.deserialize bs of
                            Right x -> do putStrLn (IR2JS.irToJSString x)
                            --                                 debugOut "deserialization OK"

                            Left s -> do
                                putStrLn "ERROR in deserialization"
                                debugOut $ "deserialization error" ++ s
                    Left s -> do
                        putStrLn "ERROR in B64 decoding"
        ...
        then exitSuccess
            ...
            if BS.isPrefixOf "!ECHO " input
                then
                    ...
                    Right bs ->
                        case CCIR.deserialize bs of
                            Right x -> BSLazyChar8.putStrLn (IR2JS.irToJSON x)
                            Left s -> do
                                putStrLn "ERROR in deserialization"
                                debugOut $ "deserialization error" ++ s
                    Left s -> do
...
main = do
  ...
  where
    compilerUsage = usageInfo header options
      where
        header = "Usage: <compiler> [OPTION...] file"
